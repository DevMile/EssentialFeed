...
class CacheFeedUseCaseTests: XCTestCase {
    ...
    func test_save_doesNotDeliverInsertionErrorAfterSUTInstanceHasBeenDeallocated() {
        ...
        var receivedResults = [LocalFeedLoader.SaveResult]()
        sut?.save(uniqueItems().models, completion: { result in
            receivedResults.append(result)
        })
    ...
    private func uniqueItems() -> (models: [FeedItem], local: [LocalFeedItem]) {
        let itemModels = [uniqueItem(), uniqueItem()]
        let localItems = itemModels.map { LocalFeedItem(id: $0.id, description: $0.description, location: $0.location, imageURL: $0.imageURL) }
        return (itemModels, localItems)
    }
